...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // QUAN TRỌNG: Luôn khởi tạo menu để menu hoạt động được
    if (typeof khoiTaoMenuDiDong === 'function') {
        khoiTaoMenuDiDong();
    }
    
    // Kiểm tra đăng nhập - nếu chưa đăng nhập, vẫn cho phép xem giỏ hàng (có thể trống)
    var currentUser = localStorage.getItem('currentUser');
    if (!currentUser) {
        // Chưa đăng nhập - vẫn hiển thị giỏ hàng (sẽ trống)
        // Không chặn code, cho phép người dùng xem trang
    }
    
    // Tải danh sách phòng trong giỏ hàng
    taiDanhSachPhongGio();
